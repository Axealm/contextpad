...
Gmailについては、実アカウントでOAuth・一覧・検索・詳細・紐づけ・保存後の保持・解除を本人が確認済みと説明できる。[受入記録](ai-dlc/19-gmail-live-acceptance.md)の範囲を超えて、再接続・失効時の実試験まで完了したとは述べない。公開用のGmail実演は専用テストアカウントと架空メールで行い、今回の実メール画像を流用しない。
