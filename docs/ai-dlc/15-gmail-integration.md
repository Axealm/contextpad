...
日付: 2026-09-14 / ローカル構築済み、Google設定と実接続検証は未完了

## Inception: 追加要求

人間の明示的な指示は「Gmail連携を優先する」「まだ作成していないので設定手順もほしい」。永続保存の先行実装は保留し、Google Cloudの準備からメール選択までを今回の作業単位とする。実接続や細部設計の承認が済んだとは記録しない。
...
| 対象 | 実装 | 制約・残課題 |
| --- | --- | --- |
| OAuth | Google公式google-auth-oauthlib、認可コード、PKCE S256 | Google設定と実接続確認は未実施 |
...
- Google設定後に実OAuth・Cookie・権限失効・再接続をユーザーと確認する。これは未完了の受入条件。
