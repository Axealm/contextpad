...
現在の目的はローカルで一連の操作を試せる試作版を作ること。Gmail連携は実装済みで、正常系7項目を実アカウントでユーザー本人が確認済み。[確認範囲と追加試験](ai-dlc/19-gmail-live-acceptance.md)を参照。メモはSQLiteへ保存する。将来は利用者認証・Bedrockによる抽出を備えた業務支援サービスにする。現行版を業務データの保管先として扱わない。
...
| FR-13 | Gmail OAuth・取得 | 接続・読み取り専用検索・10件単位の追加読込・本文確認・選択・接続解除を実装。接続から保存後の紐づけ保持・解除まで本人が実接続確認済み。追加読込・再接続・異常系の実接続結果は未報告。送信・削除・既読変更なし |
