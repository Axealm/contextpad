...
文書版: 0.1 / 更新日: 2026-09-13 / 状態: 現行実装の整理・受入レビュー待ち

## 1. 目的と課題

メールに書かれた予定や依頼と、自分の覚え書きが別々の場所にあると、作業前に文脈を読み直す必要がある。ContextPadは、メール1件と自由記述のメモを結び付け、日時・場所・人物・期限・タスクを同じ画面で確認できるようにする。

現在の目的はローカルで一連の操作を試せる試作版を作ること。将来は認証・永続保存・Gmail連携・Bedrockによる抽出を備えた業務支援サービスにする。現行版を業務データの保管先として扱わない。
...
| FR-03 | メールの紐づけ | 実装済み。1メモに0または1件。件名・送信者・本文抜粋を手入力 |
...
| FR-11 | 永続保存 | 未実装。APIプロセスのメモリにのみ保存 |
| FR-12 | 削除・履歴・共有 | 未実装。削除APIや変更履歴、ゴミ箱はない |
| FR-13 | Gmail OAuth・取得 | 設計案のみ。Googleアカウントへの接続、メール送受信は行わない |
...
| NFR-02 応答性 | UIリクエストは30秒で打ち切る | 代表データで非AI API p95 1秒以内、抽出p95 15秒以内を測定して判断 |
| NFR-03 信頼性 | プロセスメモリのみ | 永続DB、バックアップ、復旧テスト。RPO/RTOは運用前に決定 |
