...
現在は **React / TypeScript + Python / FastAPI のローカル試作版** です。Gmailの読み取り専用連携を実装しましたが、Google Cloudの認証情報は未設定で、実アカウントによる接続検証は未完了です。抽出は正規表現・キーワードによる処理で、Amazon Bedrockには接続していません。保存はAPIプロセスのメモリ上のみで、再起動すると消えます。
...
アプリ利用者の認証、永続DB、削除、共有、変更履歴、実際の生成AI呼び出し、AWSデプロイは未実装です。Gmail OAuthはアプリのログインとは別物です。**実業務メールを取り込んだり、ネットワークへ公開したりしないでください。** Terraformは一部リソースの雛形で、適用してもアプリ全体は稼働しません。
