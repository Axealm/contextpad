...
現在は **React / TypeScript + Python / FastAPI のローカル試作版** です。Gmailの読み取り専用連携は、実アカウントでのOAuth接続、一覧・検索・詳細表示、メール紐づけ、保存後の紐づけ保持、接続解除をユーザー本人が確認済みです（2026-09-14）。[確認結果と残る追加試験](docs/ai-dlc/19-gmail-live-acceptance.md)を記録しています。認証情報はGitに含まないため、別の環境では設定が必要です。抽出は正規表現・キーワードによる処理で、Amazon Bedrockには接続していません。メモはローカルのSQLiteファイルへ保存され、API再起動後も復元されます。Gmail接続用トークンは引き続きメモリ内のみです。
